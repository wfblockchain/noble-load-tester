...
if [ $platform = 'linux' ]; then
  ...
  sed -i 's/prometheus = false/prometheus = true/g' $CHAINDIR/$CHAINID/config/config.toml
...
sleep 2
nobled --home $CHAINDIR/$CHAINID $KEYRING tx tokenfactory configure-minter $(nobled --home $CHAINDIR/$CHAINID $KEYRING keys show minter -a) 1000000000$TF1_MINTING_BASEDENOM --from mintercontroller -y
sleep 2
