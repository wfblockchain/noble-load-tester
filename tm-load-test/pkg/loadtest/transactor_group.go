...
func (g *TransactorGroup) Wait() error {
	...
	for i, t := range g.transactors {
		...
		go func(_i int, _t *Transactor) {
			...
			g.trackTransactorProgress(_i, _t.GetTxCount(), _t.GetTxBytes(), _t.GetTxProcessingTime(), _t.GetBlockTime())
		}(i, t)
	}
	wg.Wait()
	// collect the results
...
func (g *TransactorGroup) trackTransactorProgress(id int, txCount int, txBytes int64, txProcessingTime float64, blockTime float64) {
	...
	g.txProcessingTime[id] = txProcessingTime
	g.blockTime[id] = blockTime
	g.statsMtx.Unlock()
}
...
func (g *TransactorGroup) reportProgress() {
	totalTxs := g.totalTxs()
	totalBytes := g.totalBytes()
	totalTxProcessingTime := g.totalTxProcessingTime()
	totalBlockTime := g.totalBlockTime()

	g.progressCallbackMtx.RLock()
	if g.progressCallback != nil {
		g.progressCallback(g, totalTxs, totalBytes, totalTxProcessingTime, totalBlockTime)
	}
